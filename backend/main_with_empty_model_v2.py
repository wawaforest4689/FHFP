...
class BackendService:
    ...
    def _sync_agent1_chat(self, req: Agent1ChatRequest) -> Dict[str, Any]:
        ...
        return {
            "success": True,
            "reply": "This is the last turn response from VAS.",
            "summary_draft": "",
    ...
    def _sync_agent1_summary(self, req: Agent1SummaryRequest) -> Dict[str, Any]:
        summary = ('''针对“宜城板鸭”品牌推广至年轻消费者的诉求，结合现有资源与核心亮点，建议以“传承与创新”为主线，融合产品力展示与故事性叙事，突出传统工艺、地道风味及现代溯源价值。具体策略如下：

### 1. 明确核心人物：爷爷辈传承人
- **角色定位**：作为传统技艺的守护者与现代化生产的见证者，老人成为贯穿视频的主角。
- **场景设计**：从乡村养鸭场到城市加工厂，展现原料筛选、腌制熏烤等关键环节；穿插特写（如晾晒鸭子、闻香反应），强化视觉辨识度。

### 2. 融合传统与现代：凸显质量把控与安全意识
- **对比逻辑**：通过老人亲自验肉、强调“自己看过的\/有标准”的细节，对比工业化生产中的潜在隐患，彰显 artisanal（手作）价值。
- **高潮点设置**：在检测/清洗环节制造紧张/惊喜转折——暗示对品质的严苛要求与最终放心口感。

### 3. 强化情感联结：突出家乡情结与代际传承
- **旁白/配乐**：选用温暖抒情风格，配合轻柔电子或民族乐器增强感染力。
- **结尾升华**：以“记得回家吃饭”等台词收束全片，唤起共鸣并引导行动转化（购买意愿）。

### 4. 配套活动规划
- **短视频挑战赛**：发起“#你吃过的最好吃的板鸭”，鼓励用户分享体验、晒出订单详情页，提高互动率与裂变传播效率。
- **KOL合作**：优先考虑与本地影响力博主或美食爱好者联名推广，放大真实口碑效应。

整体而言，该方案旨在以“爷爷的板鸭工坊”为切面，将传统工艺、食品安全与家国同频的情感内核层层递进，最终落脚于“选择优质产地、支持匠心传承”的理性号召之上。''')
        # summary="Hello,hello!"
    ...
    def _sync_agent2_generate(self, req: Agent2ShotsRequest) -> Dict[str, Any]:
        ...
        try:
            shots2=json.loads(self.escape_json_quotes(shots2.strip().removeprefix('```json').removeprefix('```').removesuffix('```').strip()))
