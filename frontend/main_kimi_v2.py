...
class MockBackendAPI(BackendAPI):
    ...
    # ------------------------------------------------------------------
    async def upload_segment_video(self, project_id: str, segment_org: VideoSegment,
        ...
        try:
            # 真实写入磁盘
            video_url = self._storage.save_segment(project_id, segment_org.segment_id, file_data, file_name)
            ...
            # Backend的segments是全局的，和UploadEditorPage不同
            """
            self._segments[project_id][segment_org.segment_id] = {
                "video_url": video_url,
                "file_name": file_name,
            ...
            segment_org.video_url=video_url
            segment_org.video_duration=vinfo.get("duration",5.0)
            self._segments[project_id][segment_org.segment_id]=segment_org

            return {
                "success": True,
                "video_url": video_url,  # 前端直接用作 <video src="...">
                "duration": vinfo.get("duration",5.0),  # 简化处理，实际可用 ffprobe 读取
                "resolution": f"{w}x{h}",
    ...
    async def update_segment_meta(self, project_id: str, segment_id: str,
                            scene: str, audio: str, text: str) -> Dict[str, Any]:
        return {"success": True, "updated_at": datetime.now().isoformat()}

    async def reorder_segments(self, project_id: str,
    ...
    async def add_segment(self, project_id: str,
    ...
    def _build_scene_annotation(self, project_id: str) -> str:
        ...
        for seg_meta in sorted_segs:
            ...
            script = json.loads(seg_meta.text)
            duration = seg_meta.video_duration
            start = current_time
            end = current_time + duration
            # seg_json = json.dumps(script, ensure_ascii=False)
...
class Agent1ChatPage(BasePage):
    ...
    async def _generate_summary(self) -> None:
        ...
        try:
            ...
            if resp.get("success"):
                ...
                # 异步模式下安全开启关闭对话框
                self.dlg=ui.dialog()
                with self.dlg, ui.card().classes('fhfp-card w-[600px] max-w-[90vw]'):
                    ...
                    with ui.card().classes('bg-green-900/30 border border-green-500/30 p-3 w-full mb-4'):
                        ...
                        ui.label(self.current_highlights).classes(
                    ...
                    with ui.row().classes('w-full justify-end gap-2'):
                        ui.button('返回修改', on_click=lambda:asyncio.create_task(self.dlg.close())).props('flat')
                        ui.button('确认并继续',
                                  on_click=lambda: self._confirm_summary(self.dlg, summary_input.value)).classes(
    ...
    async def _confirm_summary(self, dlg: ui.dialog, final_summary: str) -> None:
        """确认摘要"""
...
class Agent2EditorPage(BasePage):
    ...
    def build(self) -> None:
        ...
        with ui.column().classes('w-full min-h-screen pt-20 pb-12 px-6 max-w-6xl mx-auto'):
            ...
            # 关键修复：用 ui.timer 触发异步加载，而不是直接调用
            if not self.shots_data:
                # ui.timer(0.1, self._load_shots_async, once=True)
            ...
            with ui.card().classes('fhfp-card w-full p-5'):
                ...
                with ui.row().classes('w-full gap-4'):
                    ...
                    with ui.column().classes('flex-1'):
                        ...
                        ui.textarea(value=shot.get("text", "")).classes('w-full text-sm').style('min-height: 80px')
    ...
    def _regenerate_shots(self) -> None:
        """重新生成 - 清空数据后用 timer 触发异步加载"""
        self.shots_data = []
        if self.shots_container:
            self.shots_container.clear()
        # ui.timer(0.1, self._load_shots_async, once=True)
        asyncio.create_task(self._load_shots_async())
...
class UploadEditorPage(BasePage):
    ...
    修复点：
    1. 使用固定 preview_url，页面刷新后自动恢复已生成的视频
    2. 后台异步合成，不阻塞 NiceGUI 事件循环
    3. 合成中给用户明确反馈，减少"以为卡死而刷新"
    """
    ...
    def __init__(self):
        ...
        self._fixed_preview_url: Optional[str] = None  # 固定路径，刷新后仍可恢复

    def build(self):
        """构建完整页面 - 支持刷新后自动恢复预览"""
        self._init_segments()

        # 初始化固定预览 URL（基于项目 ID，永不丢失）
        ...
        with ui.row().classes('w-full h-screen pt-16 gap-0'):
            ...
            with ui.column().classes('flex-1 h-full overflow-y-auto p-6'):
                ...
                # 页面加载后，延迟检查是否已有预览视频（自动恢复）
                if self._fixed_preview_url:
                    ui.timer(0.5, self._try_restore_preview, once=True)
                ...
                with ui.column().classes('w-full gap-4'):
                    ...
                    with ui.card().classes('w-full p-4 bg-slate-800 border-slate-700'):
                        ...
                        with ui.row().classes('w-full items-center gap-4'):
                            ...
                            vol = ui.slider(min=0, max=10, step=0.01, value=1).classes('w-32')
                    ...
                    with ui.card().classes('w-full p-4 bg-slate-800 border-slate-700'):
                        ...
                        subtitle_switch = ui.switch('自动生成字幕', value=self.subtitle_on).classes('text-white').on_value_change(_on_subtitle_toggle)
    ...
    # 新增：刷新后自动恢复预览
    # ------------------------------------------------------------------
    def _try_restore_preview(self):
        """检查固定路径的预览文件是否存在，存在则自动恢复显示"""
        if not self._fixed_preview_url:
            return
        try:
            abs_path = self.state.backend._storage.get_absolute_path(self._fixed_preview_url)
            if abs_path.exists() and abs_path.stat().st_size > 1000:
                self.preview_url = self._fixed_preview_url
                self._update_preview()
                print(f"[INFO] 已自动恢复预览视频: {self._fixed_preview_url}")
        except Exception as e:
            print(f"[WARN] 恢复预览失败: {e}")
            ...
            if backend_segments != {}:
                ...
                for i in range(len(self.segments)):
                    ...
                    if meta and meta.video_url:
                        ...
                        self.segments[i].video_duration = meta.video_duration
            else:
                for s in existing.segments:
                    self.segments.append(VideoSegment(
                        ...
                        video_duration=getattr(s, "duration", 5.0),
                        scene=getattr(s, 'scene', ''),
            ...
            for idx, seg in enumerate(self.segments):
                with ui.card().classes('w-full p-3 bg-slate-700 border-slate-600'):
                    ...
                    with ui.column().classes('w-full gap-2 mt-2'):
                        ui.input('画面', value=seg.scene).bind_value(seg, 'scene').classes('w-full text-xs').props(
                            'dense dark')
                        ui.input('音频', value=seg.audio).bind_value(seg, 'audio').classes('w-full text-xs').props(
                            'dense dark')
                        ui.input('文案', value=seg.text).bind_value(seg, 'text').classes('w-full text-xs').props(
                            'dense dark')
    ...
    def _do_rough_cut(self):
        ...
        ui.notify("开始合成，请稍候... 合成完成后会自动显示", type="info", timeout=10000)

        # 启动后台任务（不持有 client 引用，页面刷新不影响任务继续）
        asyncio.create_task(self._run_rough_cut_task(seg_ids))

    async def _run_rough_cut_task(self, seg_ids: List[str]):
        """后台执行合成。任务在服务器端继续运行，即使用户刷新页面"""
        try:
...
def main() -> None:
    ...
    # 生产环境切换：
    # from http_backend_async import HttpBackendAPI
