...
local_model_path=r"D:/models/Qwen2.5-Omni-3B"
...
class Data_Gen():
    ...
    def video_slicing(self,video_path, length=20, threshold=27.0):
        ...
        split_video_ffmpeg(video_path, manager.get_scene_list(), output_dir=str(new_dir), show_progress=True)
    ...
    def _deploy_qwen_omni(self):
        ...
        model = Qwen2_5OmniForConditionalGeneration.from_pretrained(local_model_path,torch_dtype=torch.bfloat16,
    ...
    def valid_check(self,texts,audios,images,videos,video_files,max_new_tokens=256):
        inputs = self.processor(text=texts, audios=audios, images=images, videos=videos, return_tensors="pt",
    ...
    def agent_infer_l1(self,video_dir):
        ...
        if self.model is not None and self.processor is not None:
            inputs=self.processor(text=texts,audios=audios,images=images,videos=videos,
    ...
    """
    def generate_scene_annotation_by_video(self,video_dir,min_len=20,threshold=27.0):
        if not os.path.isdir(video_dir):
    ...
    """
    def Generate_Dataset(self,dataset_dir,L2_ds_name):
        # 音视频合成等预处理操作
        ...
        for root,dirs,files in os.walk(dataset_dir):
            for dir in dirs:
                ...
                for l in labels:
                    item={}
                    # L2层推理
                    json_array=self.generate_scene_annotation_by_video(os.path.dirname(l))
