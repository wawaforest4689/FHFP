from scenedetect import open_video,SceneManager,ContentDetector,split_video_ffmpeg,save_images
import subprocess
import os
import torch
...
class Data_Gen():
    ...
    def video_slicing(self,video_path, length=6, threshold=27.0):
        ...
        if len(os.listdir(str(new_dir)))==0:
            ...
            for i,(start,end) in enumerate(scenes):
                ...
                cli=['ffmpeg', "-i", video_path, "-ss", str(start_time), "-to", str(end_time), "-vn","-acodec","pcm_s16le",audio_name, '-y']
    ...
    def agent_infer_l2(self,scenes_dir):
        ...
        for dir in image_dirs:
            ...
            eligible=False
            while not eligible:
                outmap,eligible=self.valid_check(conversation,dir,outmap)
        return outmap

    # 大模型推理格式校验
    def valid_check(self,conversation,video_file,output_map):
        eligible=False
        try:
            """"
            completion = self.model.chat.completions.create(model="qwen2.5-omni-7b", messages=conversation,
                                                            temperature=0.85, top_p=0.9, max_tokens=384,
            ...
            completion=MultiModalConversation.call(model="qwen3.5-omni-plus",messages=conversation,
                                                   temperature=0.85,top_p=0.9,max_tokens=512,stream=True,stream_options={'include_usage':True},
                                                   incremental_output=True,enable_thinking=False)
            text=""
            for chunk in completion:
            ...
            print(f'Qwen3.5-Omni-Plus Answer:{text}\n')
        ...
        try:
            ...
            for chunk in completion:
                ...
                if chunk.output and chunk.output.choices:
                    ...
                    if isinstance(message_content, list) and len(message_content) > 0:
                        text += message_content[0]["text"]
            print(f"Qwen3-VL-Plus Answer:{text}")
            return text
        except Exception as e:
            print(f"请求错误：{e}")
        ...
        for root,dirs,files in os.walk(dataset_dir):
            for dir in dirs:
                ...
                for l in labels:
                    item={}
                    # L2层推理
                    json_array,done=self.generate_scene_annotation_by_video(os.path.dirname(l),min_len=min_len,threshold=threshold)
                    item['片段描述']=json_array
                    # 之前摘要模型是qwen3-vl-flash,效果不好需要覆盖
                    # if done:
                    #     continue
                    # L1层推理
                    ...
                    dataset.append(item)
                    # 视频级数据集写入
                    item_json_fn=os.path.join(os.path.dirname(l),"item.json")
...
if __name__=='__main__':
    ...
    data_generator.Generate_Dataset("dataset","20260523_001.jsonl",min_len=6,threshold=20.0)
